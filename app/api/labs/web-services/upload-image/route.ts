...
function buildFileName(file: File) {
  const rawName = file.name || 'product-image'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const uploadsDir = join(process.cwd(), 'public', 'uploads', 'labs', 'products')
    ...
    const publicUrl = `/uploads/labs/products/${fileNameInfo.filename}`
